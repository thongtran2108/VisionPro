...
# ═══════════════════════════════════════════════════════════════════
#  Serialize / Deserialize PatMaxModel cho .aoi embedded
#  Cho phép pipeline tự chứa model → mang sang máy khác chỉ cần file .aoi.
# ═══════════════════════════════════════════════════════════════════
def model_to_serializable(model: "PatMaxModel") -> dict:
    """Convert PatMaxModel → dict JSON-safe (meta + npz nén base64) để
    nhúng trong .aoi. Numpy arrays đi qua `np.savez_compressed` rồi
    base64 → string nhỏ gọn (tương đương file .npz trên disk)."""
    import io, base64
    np_data = {}
    for attr in ("patch_bgr", "patch_gray", "edge_image", "thumbnail", "mask"):
        arr = getattr(model, attr, None)
    ...
    if np_data:
        buf = io.BytesIO()
        np.savez_compressed(buf, **np_data)
        arrays_b64 = base64.b64encode(buf.getvalue()).decode("ascii")

    skip = {"patch_bgr", "patch_gray", "edge_image", "thumbnail", "mask",
            "precomputed_templates"}
    meta = {}
    for k, v in model.__dict__.items():
        ...
        if v is None:
            ...
            meta[k] = v
        # else skip silently
    return {"_patmax_embedded": True, "meta": meta, "arrays_b64": arrays_b64}


def model_from_serializable(d: dict) -> Optional["PatMaxModel"]:
    """Reconstruct PatMaxModel từ dict do model_to_serializable() sinh ra.
    Trả None nếu dict không đúng format."""
    import io, base64
    if not isinstance(d, dict) or not d.get("_patmax_embedded"):
        return None
    meta = d.get("meta") or {}
    try:
        model = PatMaxModel(
        ...
        b64 = d.get("arrays_b64") or ""
        if b64:
            raw = base64.b64decode(b64.encode("ascii"))
            with io.BytesIO(raw) as buf:
                npz = np.load(buf)
                for attr in ("patch_bgr", "patch_gray", "edge_image",
                             "thumbnail", "mask"):
                    if attr in npz.files:
                        setattr(model, attr, npz[attr])
        if model.train_mode == "create" and model.is_valid():
            n = precompute_templates(model)
            print(f"[PatMax embed-load] regenerated {n} DOF templates")
        return model
    except Exception as e:
        print(f"[PatMax] model_from_serializable error: {e}")
        return None
