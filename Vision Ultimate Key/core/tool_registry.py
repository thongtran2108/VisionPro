...
class _ExecContext:
    """Context cho các proc cần dữ liệu CROSS-NODE (vd Save Image composite
    overlay từ nhiều node) hoặc service ngoài (SFC scan code).

    process_fn(inputs, params) không nhận graph → runtime set context này
    TRƯỚC khi chạy: FlowGraph.execute() set `.graph`, MainWindow set `.sfc`.
    Chỉ 1 pipeline chạy 1 lúc (worker thread) nên dùng singleton an toàn.
    """
    graph = None          # FlowGraph đang execute
    sfc = None            # core.sfc.SfcManager (last_sn = mã scan gần nhất)


EXEC_CTX = _ExecContext()


def extra_pipeline_deps(node) -> set:
    """Node_ids mà `node` phụ thuộc NHƯNG không nối qua port (dữ liệu lấy từ
    output node khác qua param). FlowGraph.execute() dùng để ép các node nguồn
    chạy XONG trước:
      • save_image → overlay_nodes (composite overlay)
      • csv_log    → columns[*].node (cột link tới output node)
    """
    deps: set = set()
    tid = getattr(node, "tool_id", "")
    if tid == "save_image":
        for oid in (node.params.get("overlay_nodes") or []):
            if isinstance(oid, str):
                deps.add(oid)
    elif tid == "csv_log":
        for col in (node.params.get("columns") or []):
            if isinstance(col, dict) and col.get("kind") == "node":
                nid = col.get("node")
...
def proc_save_image(inputs, params):
    """TSaveImageTool — Lưu ảnh kết quả ra file.

    Đầu vào DUY NHẤT: port 'pass' (kết quả PASS/FAIL). Ảnh lưu KHÔNG nối qua
    port mà được DỰNG LẠI giống hệt Image Viewer "Results": ảnh gốc Acquire +
    overlay annotation của các tool đã tick trong setting (param overlay_nodes).

    Cấu trúc thư mục (theo yêu cầu):
        <save_folder>/<OK|NG>/<YYYY-MM-DD>/<tên ảnh>.<ext>
      • Tách OK/NG theo port 'pass' (split_pass_fail).
      • Thư mục con theo ngày (date_subfolder, mặc định YYYY-MM-DD).
      • Tên ảnh: theo THỜI GIAN THỰC (YYYYMMDD_HHMMSS_mmm) hoặc theo MÃ SCAN
        (SFC last_sn) — naming_mode. Mã scan trống → fallback timestamp.
    """
    import os, datetime, re

    pv = inputs.get("pass")   # kết quả pass/fail (None nếu chưa nối)

    # ── Dựng ảnh composite (ảnh gốc + overlay đã chọn) qua graph ──────
    graph = EXEC_CTX.graph
    overlay_ids = params.get("overlay_nodes") or []
    img = None
    if graph is not None:
        try:
            from core.overlay_render import render_composite
            img = render_composite(graph, overlay_ids)
        except Exception as e:
            print(f"[SaveImage] render_composite error: {e}")
    if img is None or not isinstance(img, np.ndarray):
        return {"saved": False, "path": "",
                "error": "Chưa có ảnh để lưu (Run pipeline trước)."}

    # ── Thư mục gốc ──────────────────────────────────────────────────
    # Ưu tiên save_folder_path (mới). Fallback save_path cũ (dir+filename) →
    # lấy dirname để pipeline cũ không vỡ.
    root = params.get("save_folder_path")
    if not root:
        legacy = params.get("save_path")
        if legacy:
            root = os.path.dirname(legacy) or legacy
    root = root or "output"
    ...
    if params.get("split_pass_fail", True) and pv is not None:
        ok_name = (params.get("ok_folder", "OK") or "OK").strip() or "OK"
        ng_name = (params.get("ng_folder", "NG") or "NG").strip() or "NG"
        out_dir = os.path.join(out_dir, ok_name if pv else ng_name)

    # ── Thư mục con theo ngày ────────────────────────────────────────
    mode = params.get("date_subfolder", "YYYY-MM-DD")
    if mode == "YYYY-MM-DD":
    ...
    try:
        ...
        return {"saved": False, "path": "", "error": f"mkdir failed: {e}"}

    # ── Tên ảnh: thời gian thực hoặc mã scan SFC ─────────────────────
    def _safe(s: str) -> str:
        s = re.sub(r"[^\w\-]+", "_", (s or "").strip()).strip("_")
        return s

    stamp = now.strftime("%Y%m%d_%H%M%S_") + f"{now.microsecond // 1000:03d}"
    naming = params.get("naming_mode", "Thời gian thực")
    if naming == "Mã scan (SFC)":
        sn = ""
        if EXEC_CTX.sfc is not None:
            sn = getattr(EXEC_CTX.sfc, "last_sn", "") or ""
        name = _safe(sn) or stamp   # không có mã scan → fallback timestamp
    else:
        name = stamp

    fmt = (params.get("format", "png") or "png").lower().lstrip(".")
    if fmt not in ("png", "jpg", "jpeg", "bmp", "tiff", "tif"):
        fmt = "png"   # legacy/invalid (vd "theo path") → png
    ext = "." + fmt
    enc = []
    if ext in (".jpg", ".jpeg"):
    ...
    # Tránh ghi đè khi 2 ảnh cùng tên (vd cùng mã scan) → thêm hậu tố.
    fpath = os.path.join(out_dir, f"{name}{ext}")
    if os.path.exists(fpath):
        fpath = os.path.join(out_dir, f"{name}_{stamp}{ext}")
        i = 1
        while os.path.exists(fpath):
            fpath = os.path.join(out_dir, f"{name}_{stamp}_{i}{ext}")
            i += 1

    # imencode + tofile thay cho imwrite → ghi được path Unicode (vd tên thư
    # mục/mã scan tiếng Việt) trên Windows, nơi cv2.imwrite hay fail im lặng.
    try:
        ok, buf = cv2.imencode(ext, img, enc) if enc else cv2.imencode(ext, img)
        if ok:
            buf.tofile(fpath)
    except Exception as e:
        print(f"[SaveImage] imencode/write error: {e}")
        return {"saved": False, "path": "", "error": str(e)}

    if ok:
        print(f"[SaveImage] saved: {fpath}")
        return {"saved": True, "path": fpath}
    return {"saved": False, "path": "",
            "error": "imwrite trả False — kiểm tra đường dẫn/định dạng."}

def _csv_fmt_value(v):
    """Chuẩn hoá 1 giá trị output để ghi vào bảng log: số giữ kiểu số, bool →
    TRUE/FALSE, list/ảnh → tóm tắt, còn lại → str (cắt bớt)."""
    if v is None:
        return ""
...
def _csv_build_path(params):
    """Đường dẫn file log từ folder + tên + định dạng."""
    folder = params.get("log_folder_path") or "log"
    name = (params.get("file_name") or "results").strip() or "results"
    fmt = (params.get("format", "csv") or "csv").lower()
    if fmt not in ("csv", "xlsx"):
        fmt = "csv"
    return os.path.join(os.path.abspath(folder), f"{name}.{fmt}"), fmt


def proc_csv_log(inputs, params):
    """CSV/XLSX Logger — ghi 1 HÀNG mỗi lần chạy (nhận tín hiệu ở port 'pass').

    Bảng do user tự định nghĩa qua param 'columns' (list cột):
        {"header": "Tên cột", "kind": "timestamp|pass|sn|node|const", ...}
      • timestamp → thời gian thực (timestamp_format)
      • pass      → kết quả PASS/FAIL ở port 'pass'
      • sn        → mã scan SFC (EXEC_CTX.sfc.last_sn)
      • node      → output 'port' của node 'node' (lấy qua EXEC_CTX.graph)
      • const     → giá trị cố định 'value'
    Mỗi header link tới 1 giá trị; mỗi lần Run → 1 dòng mới (giống Excel).
    Lưu .csv hoặc .xlsx (xlsx tự dựng bằng core.xlsx_io, không cần openpyxl).
    """
    import os, csv, datetime

    now = datetime.datetime.now()
    ts_fmt = params.get("timestamp_format") or "%Y-%m-%d %H:%M:%S"
    pv = inputs.get("pass")
    graph = EXEC_CTX.graph
    sfc = EXEC_CTX.sfc
    cols = params.get("columns") or []

    # Backward-compat: pipeline cũ chưa cấu hình bảng → log input scalar + time.
    if not cols:
        values = {k: v for k, v in inputs.items()
                  if isinstance(v, (int, float, bool, str))}
        values["timestamp"] = now.strftime(ts_fmt)
        headers = list(values.keys())
        row = [values[h] for h in headers]
        path = params.get("csv_path") or _csv_build_path(params)[0]
        fmt = "xlsx" if str(path).lower().endswith(".xlsx") else "csv"
    else:
        def _resolve(col):
            kind = col.get("kind", "node")
            if kind == "timestamp":
                return now.strftime(ts_fmt)
            if kind == "pass":
                return "" if pv is None else ("PASS" if pv else "FAIL")
            if kind == "sn":
                return (getattr(sfc, "last_sn", "") or "") if sfc else ""
            if kind == "const":
                return col.get("value", "")
            if kind == "node":
                nid = col.get("node")
                port = col.get("port")
                if graph is not None and nid in graph.nodes:
                    return _csv_fmt_value(graph.nodes[nid].outputs.get(port))
                return ""
            return ""

        headers = [str(c.get("header", "")) for c in cols]
        row = [_resolve(c) for c in cols]
        path, fmt = _csv_build_path(params)

    try:
        os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
        if fmt == "xlsx":
            from core.xlsx_io import append_row
            append_row(path, headers, row, sheet_name="Log")
        else:
            new = (not os.path.exists(path)) or os.path.getsize(path) == 0
            # utf-8-sig (BOM) khi tạo mới → Excel mở đúng tiếng Việt.
            ...
            with open(path, "a", newline="", encoding=enc) as f:
                ...
                if new:
                    w.writerow(headers)
                w.writerow(row)
        print(f"[CSVLog] +1 row → {path}")
        return {"logged": True, "path": path}
    except Exception as e:
        print(f"[CSVLog] error: {e}")
        return {"logged": False, "path": path, "error": str(e)}
...
# Backend inference cho YOLO Detect — user chọn .pt hay .onnx.
#   • "Auto (ONNX→PT)"  — có .onnx cùng tên thì dùng ONNX (CPU nhanh ~3-4×),
#                          không thì PyTorch. Default → giữ hành vi cũ.
#   • "PyTorch (.pt)"   — LUÔN dùng ultralytics .pt: độ chính xác gốc, đầy đủ.
#                          ONNX (FP32 + custom letterbox/NMS) có thể lệch vài
#                          box → chọn cái này nếu thấy ONNX detect kém/sai.
#   • "ONNX (fast CPU)" — ép ONNX cho tốc độ; thiếu .onnx thì tự quay về PT.
YOLO_ENGINE_MODES = ["Auto (ONNX→PT)", "PyTorch (.pt)", "ONNX (fast CPU)"]
...
def proc_yolo_detect(inputs, params):
    ...
    Backend do param `engine` quyết định:
      • "Auto (ONNX→PT)" — có `.onnx` cùng tên cạnh `.pt` thì dùng ONNX
        (CPU nhanh hơn ~3-4×), không thì PyTorch. (mặc định)
      • "PyTorch (.pt)"  — luôn chạy ultralytics `.pt`: độ chính xác gốc,
        đầy đủ. ONNX path dùng letterbox/NMS tự viết nên có thể lệch vài
        box so với `.pt` → chọn cái này khi cần accuracy tối đa.
      • "ONNX (fast CPU)" — ép ONNX Runtime (fallback cv2.dnn); thiếu
        `.onnx` thì tự quay về PyTorch.
    Segmentation models (`-seg`) luôn dùng ultralytics (cần proto-mask).
    ...
    # ── Engine selection: Auto | PyTorch (.pt) | ONNX (fast CPU) ───
    # User chọn backend qua param 'engine' (mặc định Auto = ưu tiên ONNX
    # nếu có .onnx cùng tên cho tốc độ CPU). Chọn "PyTorch (.pt)" để giữ
    # accuracy gốc của ultralytics — ONNX (FP32 + letterbox/NMS tự viết)
    # có thể lệch vài box. "ONNX (fast CPU)" thì ép dùng ONNX.
    engine = str(params.get("engine", "Auto")).lower()
    force_pt   = "pytorch" in engine or engine == "pt"
    force_onnx = engine.startswith("onnx")

    ext = os.path.splitext(model_path)[1].lower()
    onnx_candidate = (model_path if ext == ".onnx"
                      else os.path.splitext(model_path)[0] + ".onnx")
    pt_candidate = (model_path if ext == ".pt"
                    else os.path.splitext(model_path)[0] + ".pt")
    is_seg = "-seg" in os.path.basename(model_path).lower()

    # Dùng ONNX khi: không ép PT, không phải seg (seg cần proto-mask của
    # ultralytics), và có file .onnx. force_onnx mà thiếu .onnx → tự về PT.
    use_onnx = (not is_seg) and (not force_pt) and os.path.exists(onnx_candidate)
    if force_onnx and not use_onnx:
        why = ("segmentation model luôn dùng PyTorch" if is_seg
               else f"không thấy '{os.path.basename(onnx_candidate)}'")
        print(f"[YOLO] Engine=ONNX nhưng {why} — dùng PyTorch")
    ...
    # ── PyTorch path: ultralytics (.pt / seg / khi user ép PyTorch) ─
    try:
        # User ép PyTorch nhưng model_path trỏ .onnx → ưu tiên .pt thật
        # cùng tên (nếu có) để chạy đúng weights gốc, accuracy đầy đủ.
        pt_path = model_path
        if force_pt and ext == ".onnx" and os.path.exists(pt_candidate):
            pt_path = pt_candidate
        model = _yolo_load_pt(pt_path)   # cached → không nạp lại 20MB mỗi run
...
TOOL_REGISTRY: List[ToolDef] = [
  ...
  ToolDef("save_image","Save Image","Output & Display",
    "Lưu ảnh kết quả ra file. ĐẦU VÀO chỉ cần port 'pass' (kết quả PASS/FAIL). "
    "Ảnh lưu KHÔNG nối qua port — chọn 'Hiển thị kết quả' trong setting (giống "
    "Results ở Image Viewer): tick tool nào thì overlay tool đó lên ảnh gốc. "
    "Tự tách thư mục OK/NG theo pass → thư mục con theo ngày YYYY-MM-DD → ảnh. "
    "Tên ảnh theo thời gian thực hoặc theo mã scan (SFC) — TSaveImageTool",
    "#0d1117","💾",
    [PortDef("pass","bool",required=False)],
    [PortDef("saved","bool"),PortDef("path","any")],
    [P("save_folder_path","Thư mục lưu ảnh","str","output",
       tooltip="Thư mục GỐC chứa ảnh. Cấu trúc tự sinh bên trong:\n"
               "  <thư mục>/OK|NG/YYYY-MM-DD/<tên ảnh>\n"
               "Dùng đường dẫn tuyệt đối (vd D:/AOI/images) để lưu chỗ khác."),
     P("overlay_nodes","Hiển thị kết quả (overlay)","overlay_select",None,
       tooltip="Chọn tool để overlay annotation lên ảnh lưu — GIỐNG HỆT mục "
               "Results ở Image Viewer. Tick tool nào thì ảnh lưu cũng hiện "
               "annotation của tool đó. Không tick gì → lưu ảnh gốc Acquire."),
     P("naming_mode","Đặt tên ảnh theo","enum","Thời gian thực",
       choices=["Thời gian thực","Mã scan (SFC)"],
       tooltip="• Thời gian thực: tên = YYYYMMDD_HHMMSS_mmm (luôn duy nhất).\n"
               "• Mã scan (SFC): tên = mã scan gần nhất (SfcManager.last_sn). "
               "Chưa có mã scan → tự fallback về thời gian thực."),
     P("split_pass_fail","Tách thư mục OK/NG (theo pass)","bool",True,
       tooltip="Bật: ảnh PASS lưu vào .../OK, ảnh FAIL lưu vào .../NG. "
               "Cần nối port 'pass'. Port pass trống → không tách."),
     P("ok_folder","Tên thư mục PASS","str","OK",
       visible_if={"split_pass_fail":True},
       tooltip="Tên thư mục cho ảnh PASS (mặc định OK)."),
     P("ng_folder","Tên thư mục FAIL","str","NG",
       visible_if={"split_pass_fail":True},
       tooltip="Tên thư mục cho ảnh FAIL (mặc định NG)."),
     P("date_subfolder","Thư mục con theo ngày","enum","YYYY-MM-DD",
       choices=["YYYY-MM-DD","YYYY/MM/DD","YYYY-MM","Không"],
       tooltip="Gom ảnh theo ngày trong mỗi thư mục OK/NG:\n"
               "• YYYY-MM-DD → .../OK/2026-06-23/\n"
               "• YYYY/MM/DD → .../OK/2026/06/23/ (lồng nhau)\n"
               "• YYYY-MM    → .../OK/2026-06/\n"
               "• Không      → lưu thẳng, không thư mục ngày."),
     P("format","Định dạng","enum","png",
       choices=["png","jpg","bmp","tiff"],
       tooltip="Định dạng file ảnh lưu."),
  ...
  ToolDef("csv_log","CSV Logger","Output & Display",
    "Ghi kết quả ra bảng CSV/XLSX. ĐẦU VÀO chỉ cần port 'pass' — mỗi lần Run "
    "(nhận tín hiệu) ghi THÊM 1 DÒNG (như Excel). Tự định nghĩa bảng trong "
    "setting 'Bảng dữ liệu': viết header rồi link mỗi cột tới 1 giá trị "
    "(thời gian / kết quả pass-fail / mã scan SFC / output của node bất kỳ). "
    "Chọn định dạng csv hoặc xlsx.","#0d1117","📊",
    [PortDef("pass","bool",required=False)],
    [PortDef("logged","bool"),PortDef("path","any")],
    [P("columns","Bảng dữ liệu (header → giá trị)","csv_columns",None,
       tooltip="Mở trình soạn bảng: thêm cột, đặt tên header, và link mỗi cột "
               "tới giá trị cần lưu (thời gian, PASS/FAIL, mã scan, hoặc output "
               "của 1 node). Mỗi lần Run ghi 1 dòng mới theo đúng các cột này."),
     P("log_folder_path","Thư mục lưu","str","log",
       tooltip="Thư mục chứa file log. Dùng đường dẫn tuyệt đối (vd D:/AOI/log) "
               "để lưu nơi khác."),
     P("file_name","Tên file","str","results",
       tooltip="Tên file log (không cần đuôi). Đuôi theo Định dạng bên dưới. "
               "Mọi lần Run đều ghi nối vào CÙNG file này."),
     P("format","Định dạng","enum","csv",
       choices=["csv","xlsx"],
       tooltip="csv: mở bằng Excel/Notepad. xlsx: file Excel (tự dựng, không cần "
               "cài thêm thư viện)."),
     P("timestamp_format","Định dạng thời gian","str","%Y-%m-%d %H:%M:%S",
       tooltip="Định dạng cho cột kiểu 'thời gian' (strftime). Vd "
               "%Y-%m-%d %H:%M:%S → 2026-06-23 14:05:09.")],
    proc_csv_log,""),
  ...
  ToolDef("plc_write","PLC Write","Communication",
    ...
    "mode 'value' ghi 1 số vào DM/CIO/W/H. Hỗ trợ FINS/TCP, FINS/UDP, Modbus TCP, SLMP.",
    "#0b3d2e","🔌",
    [PortDef("pass","bool",required=False),
  ...
  ToolDef("yolo_detect","YOLO Detect","YOLO",
    ...
    "'Inference Engine' chọn .pt (chính xác gốc) hay .onnx (nhanh CPU). "
    ...
    [P("model_path","Model File","str","",
       tooltip="File .pt (PyTorch) hoặc .onnx (fast CPU). Backend chạy do "
               "'Inference Engine' bên dưới quyết định (mặc định Auto: có "
               ".onnx cùng tên thì ưu tiên ONNX cho tốc độ).",
       file_filter="YOLO Models (*.pt *.onnx);;PyTorch (*.pt);;"
                    "ONNX (*.onnx);;All Files (*)"),
     P("engine","Inference Engine","enum","Auto (ONNX→PT)",
       choices=YOLO_ENGINE_MODES,
       tooltip="Chọn backend chạy model:\n"
               "• Auto (ONNX→PT): có .onnx cùng tên cạnh .pt thì dùng ONNX "
               "(CPU nhanh ~3-4×), không thì PyTorch. (mặc định)\n"
               "• PyTorch (.pt): LUÔN dùng ultralytics .pt → ĐỘ CHÍNH XÁC "
               "GỐC, đầy đủ. Chọn cái này nếu ONNX detect kém/sai box.\n"
               "• ONNX (fast CPU): ép dùng ONNX cho tốc độ; thiếu .onnx thì "
               "tự quay về PyTorch."),
