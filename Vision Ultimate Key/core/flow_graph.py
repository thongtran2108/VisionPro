...
def _ancestors(start: str, in_edges: Dict[str, set]) -> set:
    """Tập node upstream (ancestor) của `start` theo in_edges. Dùng để chặn
    chu trình khi thêm dependency ẩn cho Save Image."""
    seen: set = set()
    stack = [start]
    while stack:
        cur = stack.pop()
        for dep in in_edges.get(cur, ()):
            if dep not in seen:
                seen.add(dep)
                stack.append(dep)
    return seen
...
class FlowGraph:
    ...
    def execute(self, progress_cb=None,
        ...
        # Expose graph cho proc cần dữ liệu cross-node (vd Save Image dựng ảnh
        # composite từ output của nhiều node). Chỉ 1 pipeline chạy 1 lúc.
        ...
        # Dependency ẩn: vài tool lấy dữ liệu từ output node khác QUA PARAM chứ
        # không nối port (Save Image overlay, CSV Logger cột link node) → ép node
        # nguồn chạy XONG trước. Bỏ self-ref và cạnh tạo chu trình (giữ topo hợp lệ).
        from core.tool_registry import extra_pipeline_deps
        for nid, node in nodes.items():
            for oid in extra_pipeline_deps(node):
                if oid in nodes and oid != nid and nid not in _ancestors(oid, in_edges):
                    in_edges[nid].add(oid)
                    out_edges[oid].add(nid)
