...
Hỗ trợ 4 dòng PLC:
  ...
  - Mitsubishi FX5U  (SLMP / MC Protocol 3E binary over TCP, port 5000)
...
# ── Mitsubishi FX5U — SLMP / MC Protocol 3E (binary, TCP) ─────────
# Mã thiết bị SLMP khung 3E binary: (mã 1 byte, là thiết bị bit?).
_SLMP_DEVICE_CODES = {
    ...
    'L': (0x92, True),    # Latch relay (bit)
    'B': (0xA0, True),    # Link relay (bit)
}

# Ánh xạ MemoryArea (vốn theo Omron) → thiết bị Mitsubishi. Giống cách
# InovanceH3UH5U tái dùng enum: DM_WORD↔D (data register), CIO_BIT↔M
# (internal relay) là 2 vùng dùng nhiều nhất.
_MITSU_AREA_DEVICE = {
    MemoryArea.DM_WORD:  'D',   # Data register  (word) — hay dùng nhất
    MemoryArea.DM_BIT:   'D',   # bit trong D word (read-modify-write)
    MemoryArea.CIO_WORD: 'W',   # Link register  (word)
    MemoryArea.CIO_BIT:  'M',   # Internal relay (bit)
    MemoryArea.W_WORD:   'W',   # Link register  (word)
    MemoryArea.H_WORD:   'R',   # File register  (word)
}


class MitsubishiFX5U(PLCDriver):
    """SLMP / MC Protocol 3E (binary, TCP) cho Mitsubishi FX5U (iQ-F).

    Ánh xạ vùng nhớ (dùng chung enum MemoryArea với Omron/Inovance):
        DM_WORD  → D  (data register, word) — đọc/ghi số liệu chính
        CIO_WORD → W  (link register, word)
        W_WORD   → W  (link register, word)
        H_WORD   → R  (file register, word)
        CIO_BIT  → M  (internal relay, bit) — trigger/result kiểu bit
        DM_BIT   → bit trong D word (read-modify-write)

    Cấu hình phía PLC (GX Works3): Module Parameter → Ethernet Port →
    External Device Configuration → thêm 'SLMP Connection Module',
    đặt Port (mặc định 5000), Protocol = TCP, Data Code = Binary.
    """
    ...
    @staticmethod
    def _device_bytes(device_char: str, address: int) -> bytes:
        """3 byte địa chỉ (little-endian) + 1 byte mã thiết bị."""
        code, _ = _SLMP_DEVICE_CODES[device_char]
        return struct.pack('<I', address)[:3] + struct.pack('B', code)

    def _send_recv(self, request_data: bytes) -> bytes:
        """Bọc command vào khung 3E binary, gửi & trả phần data sau end_code."""
        if not self.connected:
            self.connect()
        # Subheader + network/CPU info (cố định cho khung 3E binary)
        header = (b'\x50\x00'        # subheader (3E frame)
                  + b'\x00'          # network no (mạng cục bộ)
                  + b'\xFF'          # PC no (CPU cục bộ)
                  + b'\xFF\x03'      # dest module I/O (03FF = CPU đích)
                  + b'\x00')         # dest module station
        cpu_timer = b'\x10\x00'      # monitoring timer (16 × 250ms)
        data_len = struct.pack('<H', len(cpu_timer) + len(request_data))
        frame = header + data_len + cpu_timer + request_data
        with self._lock:
            self.sock.sendall(frame)
            head = self._recv_exact(9)            # tới hết trường 'độ dài'
            resp_len = struct.unpack('<H', head[7:9])[0]
            body = self._recv_exact(resp_len)     # end_code(2) + data
        end_code = struct.unpack('<H', body[0:2])[0]
        if end_code != 0:
            raise IOError(f"SLMP error end_code 0x{end_code:04X}")
        return body[2:]

    def read_word(self, area: MemoryArea, address: int) -> int:
        if not self.connected:
            self.connect()
        dev = _MITSU_AREA_DEVICE[self._word_area_for(area)]
        req = (b'\x01\x04' + b'\x00\x00'          # batch read, đơn vị word
               + self._device_bytes(dev, address)
               + struct.pack('<H', 1))
        data = self._send_recv(req)
        if len(data) < 2:
            raise IOError(f"SLMP short word response ({len(data)} bytes)")
    ...
    def write_word(self, area: MemoryArea, address: int, value: int) -> None:
        if not self.connected:
            self.connect()
        dev = _MITSU_AREA_DEVICE[self._word_area_for(area)]
        req = (b'\x01\x14' + b'\x00\x00'          # batch write, đơn vị word
               + self._device_bytes(dev, address)
               + struct.pack('<H', 1)
    ...
    def read_bit(self, area: MemoryArea, address: int, bit: int = 0) -> bool:
        if area == MemoryArea.CIO_BIT:
            # M (internal relay) — đọc bit gốc qua SLMP đơn vị bit
            req = (b'\x01\x04' + b'\x01\x00'      # batch read, đơn vị bit
                   + self._device_bytes(_MITSU_AREA_DEVICE[area], address)
                   + struct.pack('<H', 1))
            data = self._send_recv(req)
            if not data:
                raise IOError("SLMP empty bit response")
            return bool((data[0] >> 4) & 1)       # bit đầu = nibble cao byte 0
        # DM_BIT & các area khác: đọc D word rồi mask bit (read-modify-write).
        return super().read_bit(area, address, bit)

    def write_bit(self, area: MemoryArea, address: int, bit: int,
                  value: bool) -> None:
        if area == MemoryArea.CIO_BIT:
            # M (internal relay) — ghi bit gốc; 1 bit đóng gói ở nibble cao.
            req = (b'\x01\x14' + b'\x01\x00'      # batch write, đơn vị bit
                   + self._device_bytes(_MITSU_AREA_DEVICE[area], address)
                   + struct.pack('<H', 1)
                   + (b'\x10' if value else b'\x00'))
            self._send_recv(req)
            return
        # DM_BIT & các area khác: read-modify-write trên D word.
        super().write_bit(area, address, bit, value)
...
# Tên KIỂU TRUYỀN THÔNG (giao thức) tương ứng từng model. UI hiển thị theo nhãn
# giao thức này thay vì tên hãng/model PLC. Key = model key (GIỮ NGUYÊN để tương
# thích config đã lưu); value = nhãn giao thức.
COMM_LABEL_BY_MODEL = {
    "Omron CP2E":         "FINS/TCP",
    "Omron NX1P2":        "FINS/UDP",
    "Inovance H3U/H5U":   "Modbus TCP",
    "Mitsubishi FX5U":    "SLMP / MC Protocol 3E",
}


def comm_label(model: str) -> str:
    """Nhãn kiểu truyền thông của 1 model (fallback = chính model nếu lạ)."""
    return COMM_LABEL_BY_MODEL.get(model, model)
