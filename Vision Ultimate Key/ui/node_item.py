...
class AddTerminalDialog(QDialog):
    ...
    def __init__(self, node, parent=None, on_remove=None):
        ...
        self.setWindowTitle(tr("➕  Add Output Terminal"))
        ...
        row_obj = QHBoxLayout()
        row_obj.addWidget(QLabel(tr("Object:")))
        ...
        row_ref = QHBoxLayout()
        row_ref.addWidget(QLabel(tr("Reference:")))
        ...
        row2 = QHBoxLayout()
        row2.addWidget(QLabel(tr("Field:")))
        ...
        row3 = QHBoxLayout()
        row3.addWidget(QLabel(tr("Port name (optional):")))
        self._le_name = QLineEdit()
        self._le_name.setPlaceholderText(tr("auto: <field>  (vd: ref1_x)"))
        ...
        if existing:
            ...
            btn_remove = QPushButton(tr("🗑  Remove selected"))
        ...
        btn_row = QHBoxLayout()
        btn_row.addStretch()
        btn_cancel = QPushButton(tr("Cancel")); btn_cancel.clicked.connect(self.reject)
        btn_ok = QPushButton(tr("Add Terminal")); btn_ok.clicked.connect(self.accept)
...
class ManageOutputsDialog(QDialog):
    ...
    def __init__(self, node, parent=None, on_change=None):
        ...
        self.setWindowTitle(tr("👁  Manage Output Ports"))
        ...
        for name in port_names:
            ...
            if is_image:
                cb.setEnabled(False)
                cb.setToolTip(tr("Port 'image' luôn visible — không ẩn được "
                               "(chain port)."))
            cb.toggled.connect(
        ...
        btn_close = QPushButton(tr("Close")); btn_close.clicked.connect(self.accept)
...
class NodeItem(QGraphicsItem):
    ...
    def contextMenuEvent(self, event):
        ...
        act_run    = menu.addAction(tr("▶  Run this node"))
        menu.addSeparator()
        act_viewer = menu.addAction(tr("👁  View output in Image Viewer"))
        ...
        if self.node.tool.extra_input_type:
            menu.addSeparator()
            act_add_input = menu.addAction(tr("➕  Add Input Port"))
        ...
        if self.node.tool.extra_output_type:
            menu.addSeparator()
            act_add_output = menu.addAction(tr("➕  Add Output Port…"))
        ...
        if supports_objects:
            menu.addSeparator()
            act_add_term = menu.addAction(tr("➕  Add / Manage Output Terminals…"))
        ...
        if len(self.node.tool.outputs) >= 2:
            if not supports_objects:
                menu.addSeparator()
            act_manage = menu.addAction(tr("👁  Show / Hide Output Ports…"))
        menu.addSeparator()
