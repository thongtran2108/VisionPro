...
class CameraSetupDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle(tr("Camera Setup & Live Preview"))
    ...
    def _build_ui(self):
        ...
        hb = QHBoxLayout()
        self.btn_start = QPushButton(tr("▶  Start preview"))
        self.btn_stop  = QPushButton(tr("■  Stop"))
        self.btn_snap  = QPushButton(tr("📷  Snapshot"))
        self.btn_trigger = QPushButton(tr("⚡  Software trigger"))
        ...
        for b in (self.btn_start, self.btn_stop, self.btn_snap, self.btn_trigger):
            hb.addWidget(b)
        hb.addStretch()
        self.lbl_fps = QLabel(tr("0.0 fps"))
        ...
        self.btn_refresh = QPushButton(tr("↻  Scan"))
        ...
        gd.addWidget(QLabel(tr("Backend:")), 0, 0); gd.addWidget(self.cb_backend, 0, 1, 1, 3)
        gd.addWidget(QLabel(tr("Device:")),  1, 0); gd.addWidget(self.cb_device,  1, 1, 1, 3)
        ...
        # Image params
        self.gb_img = QGroupBox(tr("Image acquisition"))
        ...
        self.lbl_exp_rng = QLabel(tr("(open cam to see range)")); self.lbl_exp_rng.setStyleSheet(rng_style)
        ...
        gi.addWidget(QLabel(tr("Exposure:")),     0, 0); gi.addWidget(self.sp_exposure, 0, 1); gi.addWidget(self.lbl_exp_rng,  0, 2)
        gi.addWidget(QLabel(tr("Gain:")),         1, 0); gi.addWidget(self.sp_gain,     1, 1); gi.addWidget(self.lbl_gain_rng, 1, 2)
        gi.addWidget(QLabel(tr("Frame rate:")),   2, 0); gi.addWidget(self.sp_fps,      2, 1); gi.addWidget(self.lbl_fps_rng,  2, 2)
        gi.addWidget(QLabel(tr("Pixel format:")), 3, 0); gi.addWidget(self.cb_pixel_fmt, 3, 1, 1, 2)
        self.btn_apply_img = QPushButton(tr("Apply image params"))
        ...
        self.lbl_w_rng = QLabel(tr("(?)")); self.lbl_w_rng.setStyleSheet(rng_style)
        self.lbl_h_rng = QLabel(tr("(?)")); self.lbl_h_rng.setStyleSheet(rng_style)
        self.lbl_x_rng = QLabel(tr("(?)")); self.lbl_x_rng.setStyleSheet(rng_style)
        self.lbl_y_rng = QLabel(tr("(?)")); self.lbl_y_rng.setStyleSheet(rng_style)

        self.btn_apply_roi = QPushButton(tr("Apply ROI"))
        self.btn_apply_roi.clicked.connect(self._apply_roi)
        self.btn_roi_max = QPushButton(tr("Use sensor max"))
        self.btn_roi_max.setToolTip(tr("Đặt Width/Height = max của sensor, Offset = 0 (full FOV)"))
        self.btn_roi_max.clicked.connect(self._roi_use_max)
        gr.addWidget(QLabel(tr("Width:")),  0, 0); gr.addWidget(self.sp_w, 0, 1); gr.addWidget(self.lbl_w_rng, 0, 2)
        gr.addWidget(QLabel(tr("Height:")), 1, 0); gr.addWidget(self.sp_h, 1, 1); gr.addWidget(self.lbl_h_rng, 1, 2)
        gr.addWidget(QLabel(tr("OffsetX:")),2, 0); gr.addWidget(self.sp_x, 2, 1); gr.addWidget(self.lbl_x_rng, 2, 2)
        gr.addWidget(QLabel(tr("OffsetY:")),3, 0); gr.addWidget(self.sp_y, 3, 1); gr.addWidget(self.lbl_y_rng, 3, 2)
        gr.addWidget(self.btn_roi_max,   4, 0, 1, 1)
        gr.addWidget(self.btn_apply_roi, 4, 1, 1, 2)
        rv.addWidget(self.gb_roi)

        # Feature file
        gb_file = QGroupBox(tr("Feature file (.mfs)"))
        gf = QHBoxLayout(gb_file)
        self.btn_save_feat = QPushButton(tr("💾  Save…"))
        self.btn_load_feat = QPushButton(tr("📂  Load…"))
        ...
        btn_close_dlg = QPushButton(tr("Close"))
    ...
    def _open_camera(self):
        ...
        if not data:
            QMessageBox.warning(self, tr("No device"), tr("Chọn 1 device hợp lệ trước."))
            return
        self._stop_preview()
        ...
        try:
            ...
            QMessageBox.critical(self, tr("Open failed"), str(e))
    ...
    def _start_preview(self):
        if self._cam is None:
            QMessageBox.warning(self, tr("Not opened"), tr("Open camera trước."))
            return
    ...
    def _stop_preview(self):
        ...
        self.btn_snap.setEnabled(False)
        self.btn_trigger.setEnabled(False)
        self.lbl_fps.setText(tr("0.0 fps"))
