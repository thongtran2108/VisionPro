...
- Chọn kiểu truyền thông: FINS/TCP / FINS/UDP / Modbus TCP / SLMP (MC Protocol 3E)
...
from core.plc import (
    PLCManager, PLCConfig, MemoryArea, DRIVER_BY_MODEL, COMM_LABEL_BY_MODEL,
    comm_label, DataMapping, TriggerRoute,
)
...
class PLCDialog(QDialog):
    ...
    def __init__(self, manager: PLCManager, graph=None,
        ...
        self.setWindowTitle(tr("PLC Connection"))
    ...
    def _build_ui(self):
        ...
        # — Connection group —
        gb_conn = QGroupBox(tr("Connection"))
        g = QGridLayout(gb_conn)

        # Combo chọn KIỂU TRUYỀN THÔNG (hiển thị nhãn giao thức), lưu model key
        # ở itemData để tương thích config cũ + tra DRIVER_BY_MODEL.
        self.cb_model = QComboBox()
        for _m in DRIVER_BY_MODEL.keys():
            self.cb_model.addItem(COMM_LABEL_BY_MODEL.get(_m, _m), _m)
        self.cb_model.currentIndexChanged.connect(
            lambda _i: self._on_model_changed(self.cb_model.currentData()))
        ...
        g.addWidget(QLabel(tr("Kiểu truyền thông:")), 0, 0); g.addWidget(self.cb_model, 0, 1, 1, 3)
        g.addWidget(QLabel(tr("IP:")),       1, 0); g.addWidget(self.le_ip,    1, 1)
        g.addWidget(QLabel(tr("Port:")),     1, 2); g.addWidget(self.sp_port,  1, 3)
        g.addWidget(QLabel(tr("Poll:")),     2, 0); g.addWidget(self.sp_poll,  2, 1)
        ...
        self.lbl_dest_node = QLabel(tr("FINS dest node:"))
        self.lbl_src_node  = QLabel(tr("FINS src node:"))
        ...
        # của row đó.
        gb_trig = QGroupBox(
            tr("Triggers (PLC → AOI) — mỗi row 1 trigger độc lập"))
        ...
        rb = QHBoxLayout()
        self.btn_add_route = QPushButton(tr("+ Add trigger"))
        self.btn_add_route.clicked.connect(lambda: self._add_route_row())
        rb.addWidget(self.btn_add_route)
        rb.addStretch()
        self.btn_start = QPushButton(tr("▶  Start monitor"))
        self.btn_stop  = QPushButton(tr("■  Stop monitor"))
        ...
        # — Result group —
        gb_res = QGroupBox(tr("Result (AOI → PLC)"))
        ...
        gr.addWidget(QLabel(tr("Area:")),         0, 0); gr.addWidget(self.cb_res_area, 0, 1)
        gr.addWidget(QLabel(tr("Address:")),      0, 2); gr.addWidget(self.sp_res_addr, 0, 3)
        gr.addWidget(QLabel(tr("PASS value:")),   1, 0); gr.addWidget(self.sp_pass_val, 1, 1)
        gr.addWidget(QLabel(tr("FAIL value:")),   1, 2); gr.addWidget(self.sp_fail_val, 1, 3)
        ...
        gr.addWidget(QLabel(tr("Default data area:")),  2, 0); gr.addWidget(self.cb_data_area, 2, 1)
        gr.addWidget(QLabel(tr("Default start:")),      2, 2); gr.addWidget(self.sp_data_addr, 2, 3)

        self.cb_word_order = QComboBox(); self.cb_word_order.addItems(["ABCD (high word first)", "CDAB (low word first)"])
        gr.addWidget(QLabel(tr("Float/int32 word order:")), 3, 0); gr.addWidget(self.cb_word_order, 3, 1, 1, 3)
        ...
        self.cb_judge_node.setToolTip(
            tr("Chọn node có port 'pass' để quyết định OK/NG gửi PLC. "
            "Auto = pass khi mọi node trong pipeline pass."))
        gr.addWidget(QLabel(tr("Judge node:")), 4, 0)
        gr.addWidget(self.cb_judge_node, 4, 1, 1, 3)

        self.btn_send_pass = QPushButton(tr("Send PASS (test)"))
        self.btn_send_fail = QPushButton(tr("Send FAIL (test)"))
        ...
        gb_map = QGroupBox(tr("Data mappings (output của node → vùng nhớ PLC)"))
        ...
        hb_map = QHBoxLayout()
        self.btn_add_map  = QPushButton(tr("➕  Add mapping"))
        self.btn_test_map = QPushButton(tr("⇡  Test send mappings"))
        self.btn_refresh_map = QPushButton(tr("↻  Refresh node list"))
        ...
        gb_sfc = QGroupBox(tr("SFC Trigger Sequence (PLC trigger → Scan → GET → Pipeline → POST)"))
        gs = QGridLayout(gb_sfc)

        self.chk_sfc_seq_enabled = QCheckBox(
            tr("Enable SFC sequence khi PLC trigger"))
        self.chk_sfc_seq_enabled.setToolTip(
            tr("Bật: PLC fire trigger DM → tự động chạy Scan → API GET → "
            "Pipeline → API POST.\n"
            "Tắt: PLC trigger chỉ chạy Pipeline (behaviour cũ)."))
        gs.addWidget(self.chk_sfc_seq_enabled, 0, 0, 1, 4)

        self.chk_sfc_step_scan = QCheckBox(tr("• Bước 1: Scan barcode/QR (Scanner section của SFC)"))
        self.chk_sfc_step_scan.setChecked(True)
        self.chk_sfc_step_scan.setToolTip(
            tr("Bỏ tích nếu PLC trigger không cần scan (vd SN đã có sẵn từ camera/QR khác)."))
        gs.addWidget(self.chk_sfc_step_scan, 1, 0, 1, 4)

        self.chk_sfc_step_get = QCheckBox(tr("• Bước 2: API GET (check SN trên SFC)"))
        self.chk_sfc_step_get.setChecked(True)
        self.chk_sfc_step_get.setToolTip(
            tr("Gọi API GET với {SN} từ bước scan. Tắt nếu không cần lookup trước "
            "khi chạy pipeline."))
        gs.addWidget(self.chk_sfc_step_get, 2, 0, 1, 4)

        self.chk_sfc_step_post = QCheckBox(tr("• Bước 4: API POST (gửi kết quả sau pipeline)"))
        self.chk_sfc_step_post.setChecked(True)
        self.chk_sfc_step_post.setToolTip(
            tr("Sau khi pipeline run xong → tự động POST kết quả với body template "
            "đã cấu hình. Tắt nếu chỉ muốn lưu local."))
        gs.addWidget(self.chk_sfc_step_post, 3, 0, 1, 4)

        self.chk_sfc_abort_on_fail = QCheckBox(
            tr("Dừng sequence nếu Scan/GET fail (không chạy pipeline & POST)"))
        self.chk_sfc_abort_on_fail.setChecked(True)
        self.chk_sfc_abort_on_fail.setToolTip(
            tr("Bật: scan timeout / GET trả status ≠ expected → abort, không "
            "chạy pipeline.\n"
            "Tắt: vẫn chạy pipeline kể cả khi scan/GET fail."))
        gs.addWidget(self.chk_sfc_abort_on_fail, 4, 0, 1, 4)

        btn_open_sfc = QPushButton(tr("⚙  Open SFC configuration…"))
        btn_open_sfc.setToolTip(tr("Mở dialog cấu hình Scanner / API GET / API POST"))
        ...
        # — Log —
        self.lbl_status = QLabel(tr("● Disconnected"))
    ...
    def _add_route_row(self, route: Optional[TriggerRoute] = None):
        ...
        # Delete
        btn_del = QPushButton(tr("✖")); btn_del.setFixedWidth(28)
    ...
    def _test_send_mappings(self):
        ...
        if not self._mgr.is_connected:
            QMessageBox.warning(self, tr("Not connected"), tr("Connect PLC trước đã."))
            return
        if self._graph is None:
            QMessageBox.warning(self, tr("No graph"), tr("Không có pipeline."))
            return
        ...
        try:
            ...
            QMessageBox.critical(self, tr("Write failed"), str(e))
            return
        for r in report:
            if "error" in r:
    ...
    def _on_model_changed(self, model: str):
        ...
        elif "Mitsubishi" in model or "FX5U" in model:
            self.sp_port.setValue(5000)

    def _gather_config(self) -> PLCConfig:
        return PLCConfig(
            model=self.cb_model.currentData() or self.cb_model.currentText(),
    ...
    def _apply_config_to_ui(self, cfg: PLCConfig):
        # Tìm theo model key (itemData). Fallback findText cho config rất cũ
        # từng lưu nhãn hiển thị thay vì key.
        idx = self.cb_model.findData(cfg.model)
        if idx < 0:
            idx = self.cb_model.findText(cfg.model)
        if idx >= 0:
            self.cb_model.setCurrentIndex(idx)
        # Gọi trước khi set port để default-port theo model không ghi đè cfg.port.
    ...
    def _refresh_status(self):
        if self._mgr.is_connected:
            self.lbl_status.setText(
                f"● Connected via {comm_label(self._mgr.config.model)} "
                f"@ {self._mgr.config.ip}"
            ...
            self.lbl_status.setText(tr("● Disconnected"))
            self.lbl_status.setStyleSheet("color:#ff3860;font-weight:700;")
    ...
    def _on_connect(self):
        ...
        try:
            self._mgr.connect()
            self._log(f"Connected via {comm_label(self._mgr.config.model)} "
                      f"@ {self._mgr.config.ip}:{self._mgr.config.port}")
        except Exception as e:
            QMessageBox.critical(self, tr("Connect failed"), str(e))
    ...
    def _on_start_monitor(self):
        ...
        try:
            ...
            QMessageBox.critical(self, tr("Start monitor failed"), str(e))
        self._refresh_status()
        self._save_settings()
    ...
    def _on_open_sfc_dialog(self):
        ...
        if parent is not None and hasattr(parent, "_open_sfc_dialog"):
            ...
            QMessageBox.information(self, tr("SFC"),
                tr("Mở Tools → SFC / MES Integration… từ main window để cấu hình."))
