...
class RecentFileCard(QWidget):
    ...
    def __init__(self, path: str, is_default: bool = False, parent=None):
        ...
        self._default_badge = QLabel(tr("⭐  DEFAULT"))
...
class StartupAOIPicker(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle(tr("VisionUltimate — Load AOI"))
    ...
    def _build_ui(self):
        ...
        title = QLabel("VisionUltimate AOI")
        title.setStyleSheet(
            "color:#00d4ff;font-size:26px;font-weight:800;letter-spacing:2px;")
        hero_text.addWidget(title)
        sub = QLabel(tr("Chọn file AOI gần đây để tiếp tục, hoặc bắt đầu mới."))
        ...
        btn_browse.setToolTip(tr("Mở file .aoi/.json bất kỳ qua file dialog."))
        btn_browse.clicked.connect(self._on_browse)
        btn_row.addWidget(btn_browse)

        btn_new = QPushButton(tr("✨  New blank project"))
        btn_new.setObjectName("ghost")
        btn_new.setToolTip(tr("Tạo pipeline trống, không load file."))
        btn_new.clicked.connect(self._on_new)
        btn_row.addWidget(btn_new)

        btn_row.addStretch(1)

        self._btn_open = QPushButton(tr("Open Selected →"))
        self._btn_open.setObjectName("primary")
        self._btn_open.setToolTip(tr("Mở file đang chọn trong recent list. "
    ...
    def _populate_recents(self):
        ...
        if not recents:
            ...
            msg = QLabel(tr("Chưa có file AOI nào gần đây"))
    ...
    def _on_browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Open AOI File", projects_dir(),
