...
class YoloStudioDialog(QDialog):
    ...
    def __init__(self, parent=None, initial_image=None):
        super().__init__(parent)
        self.setWindowTitle(tr("🤖  YOLO Studio  —  Label · Dataset · Train"))
    ...
    def _build_ui(self):
        ...
        # Project info
        self._proj_lbl = QLabel(tr("No project open"))
        ...
        self._ann_count_lbl = QLabel(tr("Annotations: 0"))
        ...
        """)
        self._tabs.addTab(self._build_label_tab(),   tr("✏  Label"))
        self._tabs.addTab(self._build_dataset_tab(), tr("📦  Dataset"))
    ...
    def _build_label_tab(self) -> QWidget:
        ...
        # Mode
        mode_grp = QGroupBox(tr("Draw Mode"))
        ...
        # Save button
        self._btn_save_label = QPushButton(tr("💾  Save Labels"))
    ...
    def _build_dataset_tab(self) -> QWidget:
        ...
        # Top: project info
        info_grp = QGroupBox(tr("Project Info"))
        ig = QVBoxLayout(info_grp); ig.setContentsMargins(10,12,10,10); ig.setSpacing(6)
        self._ds_info = QLabel(tr("Open a project to see dataset info."))
        ...
        btn_split = QPushButton(tr("⚡ Auto Split"))
        ...
        # YAML generator
        yaml_grp = QGroupBox(tr("data.yaml Generator"))
        ...
        # Dataset stats table
        stat_grp = QGroupBox(tr("Label Statistics"))
    ...
    def _build_train_tab(self) -> QWidget:
        ...
        # Model selection
        model_grp = QGroupBox(tr("Base Model"))
        ...
        hyp_grp = QGroupBox(tr("Hyperparameters"))
        ...
        self._hp_val_tta = QCheckBox(tr("Validation TTA  (chậm hơn, mAP nhỉnh hơn)"))
        self._hp_val_tta.setChecked(False)
        self._hp_val_tta.setToolTip(
            tr("Test-Time Augmentation cho validation phase. Training "
        ...
        # Train / Stop buttons
        self._btn_start_train = QPushButton(tr("🚀  Start Training"))
        ...
        self._btn_stop_train = QPushButton(tr("■  Stop"))
        ...
        self._train_status = QLabel(tr("Idle"))
        ...
        self._live_grp = QGroupBox(tr("Live Metrics"))
        ...
        self._live_losses = QLabel(tr("box_loss: —   cls_loss: —   dfl_loss: —"))
        ...
        self._live_chart_lbl = QLabel(tr("(chart sẽ hiện sau epoch đầu tiên)"))
        ...
        # Log
        log_lbl = QLabel(tr("TRAINING LOG"))
        ...
        self._result_grp = QGroupBox(tr("Last Training Result"))
        rg2 = QVBoxLayout(self._result_grp); rg2.setContentsMargins(8,12,8,8)
        self._result_info = QLabel(tr("—"))
        ...
        self._eval_grp = QGroupBox(tr("📊 Model Evaluation"))
        ...
        self._btn_export_onnx.setToolTip(
            tr("Export best.pt → best.onnx (FP32) cho CPU inference nhanh hơn 3-4x.\n"
    ...
    def _new_project(self):
        ...
        if base.exists():
            QMessageBox.warning(self,tr("Exists"),f"'{base}' already exists.")
            return
        try:
            ...
            QMessageBox.information(self,tr("Created"),f"Project created:\n{base}")
        except Exception as e:
    ...
    def _use_aoi_image(self):
        ...
        if self._initial_image is not None and isinstance(self._initial_image, np.ndarray):
            ...
            self._status_lbl.setText(tr("Loaded image from AOI pipeline."))
        else:
            QMessageBox.information(self,tr("Info"),
                tr("Mở YOLO Studio từ node AOI có output image để dùng tính năng này."))
    ...
    def _auto_split(self):
        if not self._project_dir:
            QMessageBox.warning(self,tr("No Project"),tr("Open a project first."))
            return
    ...
    def _start_training(self):
        ...
        if not yaml_path or not os.path.exists(yaml_path):
            QMessageBox.warning(self,tr("Missing YAML"),
                tr("Chọn data.yaml trước. Vào tab Dataset → Generate data.yaml."))
            return
        ...
        self._live_chart_lbl.setText(tr("(chart sẽ hiện sau epoch đầu tiên)"))
    ...
    def _stop_training(self):
        ...
        self._train_status.setText(tr("Stopped"))
    ...
    def _on_train_done(self, save_dir: str):
        ...
        self._finalize_training()
        self._train_status.setText(tr("✅ Done!"))
        ...
        QMessageBox.information(self,tr("Training Complete"),
    ...
    def _populate_charts(self, save_dir: str):
        ...
        for fn, title in chart_files:
            ...
            added += 1
        if added == 0:
            empty = QLabel(tr("(Chưa có chart — train chưa sinh results.png)"))
    ...
    def _on_train_error(self, msg: str):
        self._finalize_training()
        self._train_status.setText(tr("Error!"))
        self._train_status.setStyleSheet("color:#ff3860; font-size:11px;")
        self._train_log.append(f"\n❌ ERROR: {msg}")
        QMessageBox.critical(self, tr("Training Error"), _explain_train_error(msg))
    ...
    def _export_onnx_clicked(self):
        ...
        if not d:
            QMessageBox.warning(self, tr("No Model"), tr("Chưa có training nào hoàn tất."))
            return
        best_pt = os.path.join(d, "weights", "best.pt")
        if not os.path.exists(best_pt):
            QMessageBox.warning(self, tr("Missing best.pt"),
                f"Không tìm thấy:\n{best_pt}")
            return
        ...
        if missing:
            ...
            r = QMessageBox.question(self, tr("ONNX packages missing"), msg,
    ...
    def _on_export_done(self, ok: bool, msg: str):
        ...
        if ok:
            ...
            QMessageBox.information(self, tr("Export OK"),
                f"ONNX exported:\n{msg}")
        else:
    ...
    def _save_model_to(self):
        ...
        for fn in ("best.pt", "best.onnx", "best.classes.txt"):
            ...
            if os.path.exists(p):
                candidates.append(p)
        if not candidates:
            QMessageBox.warning(self, tr("Not Found"),
                tr("Không tìm thấy file model nào trong weights/."))
            return
        ...
        QMessageBox.information(self, tr("Model Saved"),
