...
class SfcDialog(QDialog):
    ...
    def __init__(self, manager: SfcManager, graph=None,
        ...
        super().__init__(parent)
        self.setWindowTitle(tr("SFC / MES Integration"))
    ...
    def _build_ui(self):
        ...
        # ── 1. Scanner ────────────────────────────────────────────
        gb_scan = QGroupBox(tr("1.  Hand Scanner  (Serial / COM)"))
        g = QGridLayout(gb_scan)

        self.cb_port = QComboBox()
        self.cb_port.setMinimumWidth(280)
        self.btn_refresh_ports = QPushButton(tr("🔄  Refresh"))
        ...
        self.le_trigger.setPlaceholderText(tr("Hex bytes (vd 02 F4 03). Để trống nếu scanner auto-trigger."))
        ...
        self.le_contains.setPlaceholderText(tr("(tuỳ chọn) data phải chứa chuỗi này"))

        row = 0
        g.addWidget(QLabel(tr("COM port:")), row, 0); g.addWidget(self.cb_port, row, 1, 1, 2); g.addWidget(self.btn_refresh_ports, row, 3); row += 1
        g.addWidget(QLabel(tr("Baudrate:")), row, 0); g.addWidget(self.sp_baud, row, 1)
        g.addWidget(QLabel(tr("Byte:")), row, 2); g.addWidget(self.cb_bytesize, row, 3); row += 1
        g.addWidget(QLabel(tr("Parity:")), row, 0); g.addWidget(self.cb_parity, row, 1)
        g.addWidget(QLabel(tr("Stop bits:")), row, 2); g.addWidget(self.cb_stopbits, row, 3); row += 1
        g.addWidget(QLabel(tr("Timeout:")), row, 0); g.addWidget(self.sp_timeout, row, 1)
        g.addWidget(QLabel(tr("Read size:")), row, 2); g.addWidget(self.sp_readsize, row, 3); row += 1
        g.addWidget(QLabel(tr("Trigger (hex):")), row, 0); g.addWidget(self.le_trigger, row, 1, 1, 3); row += 1
        g.addWidget(QLabel(tr("Encoding:")), row, 0); g.addWidget(self.cb_encoding, row, 1)
        g.addWidget(QLabel(tr("Expected len:")), row, 2); g.addWidget(self.sp_expected_len, row, 3); row += 1
        g.addWidget(QLabel(tr("Contains:")), row, 0); g.addWidget(self.le_contains, row, 1, 1, 3); row += 1

        self.btn_scan_test = QPushButton(tr("📟  Test Scan"))
        self.btn_scan_test.clicked.connect(self._on_scan_test)
        self.lbl_last_sn = QLabel(tr("Last SN: (chưa scan)"))
        ...
        # ── 2. API GET ────────────────────────────────────────────
        gb_get = QGroupBox(tr("2.  API GET  (lookup SN trên SFC)"))
        gg = QGridLayout(gb_get)
        self.chk_get_enabled = QCheckBox(tr("Enable API GET"))
        gg.addWidget(self.chk_get_enabled, 0, 0, 1, 4)

        self.le_get_url = QLineEdit("")
        self.le_get_url.setPlaceholderText(
            tr("Vd: http://10.222.48.213:8888/v2/pass/mes/tsc/check/TSC-VN/TSC_VN1/OPLEUATTACHMENT?sn={SN}&station_id=OPLEUATTACHMENT"))
        gg.addWidget(QLabel(tr("URL template:")), 1, 0)
        gg.addWidget(self.le_get_url, 1, 1, 1, 3)

        self.le_get_headers = QLineEdit("{}")
        self.le_get_headers.setPlaceholderText(tr('{"token":"abc","X-API-Key":"xxx"}'))
        gg.addWidget(QLabel(tr("Headers (JSON):")), 2, 0)
        ...
        gg.addWidget(QLabel(tr("Timeout:")), 3, 0); gg.addWidget(self.sp_get_timeout, 3, 1)
        gg.addWidget(QLabel(tr("Expected status:")), 3, 2); gg.addWidget(self.sp_get_status, 3, 3)

        self.le_get_expected = QLineEdit("")
        self.le_get_expected.setPlaceholderText(tr("(tuỳ chọn) response.text phải chứa chuỗi này → pass"))
        gg.addWidget(QLabel(tr("Must contain:")), 4, 0)
        gg.addWidget(self.le_get_expected, 4, 1, 1, 3)

        self.chk_get_parse_json = QCheckBox(tr("Parse JSON"))
        self.le_get_json_path   = QLineEdit("")
        self.le_get_json_path.setPlaceholderText(tr('Dot path, vd "data.token"'))
        gg.addWidget(self.chk_get_parse_json, 5, 0)
        gg.addWidget(QLabel(tr("JSON path:")), 5, 1); gg.addWidget(self.le_get_json_path, 5, 2, 1, 2)

        self.chk_get_bypass_proxy = QCheckBox(tr("Bypass system proxy (cho IP nội bộ LAN)"))
        self.chk_get_bypass_proxy.setChecked(True)
        self.chk_get_bypass_proxy.setToolTip(
            tr("Bật để bỏ qua HTTP_PROXY / corporate Squid proxy — tránh "
            "request về IP LAN (10.x, 192.168.x) bị proxy chặn / trả HTML lỗi."))
        gg.addWidget(self.chk_get_bypass_proxy, 6, 0, 1, 4)

        self.btn_get_test = QPushButton(tr("🌐  Test GET"))
        ...
        self.txt_get_resp.setPlaceholderText(tr("Response sẽ hiện ở đây sau khi Test GET"))
        gg.addWidget(self.txt_get_resp, 8, 0, 1, 4)
        root.addWidget(gb_get)

        # ── 3. API POST ───────────────────────────────────────────
        gb_post = QGroupBox(tr("3.  API POST  (gửi kết quả sau pipeline)"))
        gp = QGridLayout(gb_post)
        self.chk_post_enabled = QCheckBox(tr("Enable API POST"))
        gp.addWidget(self.chk_post_enabled, 0, 0, 1, 4)

        self.le_post_url = QLineEdit("")
        self.le_post_url.setPlaceholderText(tr("Vd: http://10.222.48.213:8888/v2/sfc/result"))
        gp.addWidget(QLabel(tr("URL:")), 1, 0)
        ...
        gp.addWidget(QLabel(tr("Method:")), 2, 0); gp.addWidget(self.cb_post_method, 2, 1)
        gp.addWidget(QLabel(tr("Timeout:")), 2, 2); gp.addWidget(self.sp_post_timeout, 2, 3)

        self.le_post_headers = QLineEdit('{"Content-Type":"application/json"}')
        self.le_post_headers.setPlaceholderText(tr('Vd: {"Content-Type":"application/json","token":"abc"}'))
        gp.addWidget(QLabel(tr("Headers (JSON):")), 3, 0)
        gp.addWidget(self.le_post_headers, 3, 1, 1, 3)

        gp.addWidget(QLabel(tr("Expected status:")), 4, 0); gp.addWidget(self.sp_post_status, 4, 1)
        self.le_post_expected = QLineEdit("")
        self.le_post_expected.setPlaceholderText(tr("(tuỳ chọn) response phải chứa chuỗi này"))
        gp.addWidget(QLabel(tr("Must contain:")), 4, 2); gp.addWidget(self.le_post_expected, 4, 3)

        self.chk_post_bypass_proxy = QCheckBox(tr("Bypass system proxy (cho IP nội bộ LAN)"))
        self.chk_post_bypass_proxy.setChecked(True)
        self.chk_post_bypass_proxy.setToolTip(
            tr("Bật để bỏ qua HTTP_PROXY / corporate Squid proxy."))
        gp.addWidget(self.chk_post_bypass_proxy, 5, 0, 1, 4)

        # Body template — JSON editor + insert reference helper
        hdr_body = QHBoxLayout()
        hdr_body.addWidget(QLabel(tr("Body template (JSON):")))
        hdr_body.addStretch()
        ...
        self.btn_insert_ref = QPushButton(tr("➕  Insert {ref}"))
        self.btn_insert_ref.clicked.connect(self._on_insert_ref)
        hdr_body.addWidget(QLabel(tr("Reference:"))); hdr_body.addWidget(self.cb_node_pick)
        ...
        self.btn_post_test = QPushButton(tr("📤  Test POST"))
        self.btn_post_test.clicked.connect(self._on_post_test)
        self.btn_post_preview = QPushButton(tr("👁  Preview body"))
        ...
        self.txt_post_resp.setPlaceholderText(tr("Response sẽ hiện ở đây sau khi Test POST"))
    ...
    def _on_scan_test(self):
        ...
        if err:
            QMessageBox.warning(self, tr("Scan"), f"Lỗi: {err}")
            return
        if not sn:
            QMessageBox.information(self, tr("Scan"), tr("Không nhận được data (timeout?)"))
            return
    ...
    def _on_save(self):
        self._apply_to_manager()
        self._save_settings()
        QMessageBox.information(self, tr("Saved"), tr("Đã lưu cấu hình SFC."))
