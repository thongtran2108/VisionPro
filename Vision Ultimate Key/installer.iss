...
;      → tạo ra folder dist\VisionUltimate\
;
;   2. Tải Inno Setup:
...
#define MyAppPublisher "VisionUltimate"
...
DefaultDirName={autopf}\VisionUltimate
...
[Files]
; ── Toàn bộ folder dist\VisionUltimate\ ─────────────────────────────
; recursesubdirs: bao gồm sub-folder (assets, vendor, _internal\dll, ...)
; createallsubdirs: tạo cả folder rỗng.
; Excludes: KHÔNG nhúng license/secret vào installer giao khách (mỗi máy
;           tự kích hoạt; *.pem là tàn dư mô hình key cũ nếu còn sót).
Source: "dist\VisionUltimate\*"; DestDir: "{app}"; \
    Excludes: "*.pem,license.key"; \
    Flags: ignoreversion recursesubdirs createallsubdirs

; ── Camera SDK DLL (MVS HikRobot) — copy CẠNH exe để app tự dò ──────
; Lấy trực tiếp từ folder dll\ của project (bỏ DLL của MVS vào đó). App
; tìm {app}\dll TRƯỚC → camera chạy mà KHÔNG cần cài MVS SDK trên máy đích.
; skipifsourcedoesntexist: bỏ qua nếu chưa tạo folder dll\.
Source: "dll\*"; DestDir: "{app}\dll"; Excludes: "*.md"; \
    Flags: ignoreversion recursesubdirs createallsubdirs skipifsourcedoesntexist

; ── Optional: kèm MVS SDK installer nếu KHÔNG bundle DLL như trên ───
; (Khuyến nghị dùng cách bundle DLL ở trên thay vì bắt khách cài SDK.)
...
begin
  if not IsWin64 then
  begin
    MsgBox('AOI Vision Ultimate yêu cầu Windows 64-bit.' + #13#10 +
