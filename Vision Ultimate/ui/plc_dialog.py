...
- Chọn loại PLC: Omron CP2E / Omron NX1P2 / Inovance H3U-H5U /
  Mitsubishi FX5U / Panasonic FP
- Cấu hình IP, port, polling interval (Panasonic FP serial: 2 field này
  thành "COM port" + "Baud")
...
class PLCDialog(QDialog):
    ...
    def __init__(self, manager: PLCManager, graph=None,
        ...
        # True trong lúc nạp settings → _on_model_changed không tự ghi đè
        # IP/Port/Baud đã lưu bằng giá trị default.
    ...
    def _build_ui(self):
        ...
        # Max 999999 để chứa cả baud rate (vd 115200) khi field này dùng làm
        # "Baud" cho Panasonic FP serial — TCP port thực tế vẫn ≤ 65535.
        self.sp_port = QSpinBox(); self.sp_port.setRange(1, 999999); self.sp_port.setValue(9600)
        self.sp_poll = QSpinBox(); self.sp_poll.setRange(10, 5000); self.sp_poll.setValue(100); self.sp_poll.setSuffix(" ms")

        # Lưu ref để đổi nhãn IP/Port → COM port/Baud khi chọn PLC serial.
        self.lbl_ip = QLabel("IP:")
        self.lbl_port = QLabel("Port:")
        g.addWidget(QLabel("Model:"),    0, 0); g.addWidget(self.cb_model, 0, 1, 1, 3)
        g.addWidget(self.lbl_ip,         1, 0); g.addWidget(self.le_ip,    1, 1)
        g.addWidget(self.lbl_port,       1, 2); g.addWidget(self.sp_port,  1, 3)
    ...
    def _on_model_changed(self, model: str):
        ...
        # Panasonic FP dùng cổng COM (serial) → đổi nhãn IP/Port thành
        # COM port/Baud. Các PLC còn lại là Ethernet → nhãn IP/Port.
        is_serial = "Panasonic" in model
        if is_serial:
            self.lbl_ip.setText("COM port:")
            self.lbl_port.setText("Baud:")
            self.le_ip.setToolTip(
                "Tên cổng COM, vd 'COM6' (Windows) hoặc '/dev/ttyUSB0' (Linux)")
            self.sp_port.setToolTip("Baud rate, vd 9600 / 19200 / 115200")
        else:
        ...
        # Trong lúc nạp settings: chỉ đổi nhãn, KHÔNG ghi đè giá trị đã lưu.
        if self._loading_settings:
            return

        # User tự đổi model → đặt port/baud mặc định + ví dụ địa chỉ hợp lý.
        if is_serial:
            self.sp_port.setValue(9600)
            if self._looks_like_ip(self.le_ip.text()):
                self.le_ip.setText("COM1")     # IP cũ vô nghĩa cho serial
        else:
            if not self._looks_like_ip(self.le_ip.text()):
                self.le_ip.setText("192.168.250.1")  # COM cũ vô nghĩa cho Ethernet
            if is_nx or "CP2E" in model:
                self.sp_port.setValue(9600)
            elif "H3U" in model or "H5U" in model:
                self.sp_port.setValue(502)
            elif "Mitsubishi" in model:
    ...
    def _apply_config_to_ui(self, cfg: PLCConfig):
        # Khoá auto-default trong lúc nạp để giữ nguyên IP/Port/Baud đã lưu
        # (setCurrentIndex bên dưới sẽ fire _on_model_changed).
        self._loading_settings = True
        try:
            self._apply_config_to_ui_inner(cfg)
        finally:
            self._loading_settings = False

    def _apply_config_to_ui_inner(self, cfg: PLCConfig):
