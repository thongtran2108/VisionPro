...
class ConnectionItem(QGraphicsPathItem):
    ...
    def contextMenuEvent(self, event):
        ...
        menu.addSeparator()
        act_unlink = menu.addAction(tr("✂  Unlink (xoá kết nối)"))
        menu.addSeparator()

        # Re-link: chọn node + port khác để nối lại đầu nguồn / đầu đích.
        src_menu = menu.addMenu(tr("⬅  Link nguồn tới node…"))
        dst_menu = menu.addMenu(tr("➡  Link đích tới node…"))
...
class AOIScene(QGraphicsScene):
    ...
    def __init__(self, graph: FlowGraph, parent=None):
        ...
        self._signals.delete_req.connect(self._on_delete_req)
        ...
        # Khoá chỉnh sửa cấu trúc (di chuyển/nối dây/xoá node) khi chưa đăng
        # nhập. Vẫn cho chọn/zoom/pan để XEM. Phải set TRƯỚC _load_graph vì
        # _add_node_item đọc cờ này để bật/tắt movability.
        self._editable = True

        self._load_graph()

    def set_editable(self, editable: bool):
        """Bật/tắt chỉnh sửa pipeline trên canvas. Khoá → node không kéo
        được, không nối/xoá dây, không xoá node; vẫn chọn/zoom/pan để xem."""
        self._editable = bool(editable)
        for ni in self._node_items.values():
            ni.setFlag(QGraphicsItem.ItemIsMovable, self._editable)

    def is_editable(self) -> bool:
        return self._editable

    def _on_delete_req(self, node_id: str):
    ...
    def _add_node_item(self, node) -> NodeItem:
        ...
        item.setFlag(QGraphicsItem.ItemIsMovable, self._editable)
        self.addItem(item)
