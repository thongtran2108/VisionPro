...
class ToolButton(QFrame):
    def __init__(self, tool: ToolDef, parent=None):
        ...
        tip = f"<b>{tr(tool.name)}</b>"
        if tool.T_equiv:
            tip += f"<br><i style='color:#00d4ff'>{tool.T_equiv}</i>"
        tip += f"<br>{tr(tool.description)}"
        self.setToolTip(tip)
        ...
        top_row = QLabel(f"{tool.icon}  {tr(tool.name)}")
        ...
        if tool.T_equiv:
            ...
            _desc = tr(tool.description)
            bot_row = QLabel(_desc[:42] + ("…" if len(_desc) > 42 else ""))
            bot_row.setStyleSheet(
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        if (self._drag_start and
            ...
            p.drawText(pix.rect(), Qt.AlignCenter, f"{self.tool.icon} {tr(self.tool.name)}")
            p.end()
...
class CategorySection(QWidget):
    ...
    def filter(self, text: str):
        ...
        for btn in self._buttons:
            vis = (t in btn.tool.name.lower() or
                   t in tr(btn.tool.name).lower() or
                   t in btn.tool.description.lower() or
                   t in tr(btn.tool.description).lower() or
                   t in btn.tool.T_equiv.lower())
            btn.setVisible(vis)
...
class ToolLibraryPanel(QWidget):
    def __init__(self, parent=None):
        ...
        self._search.setPlaceholderText(tr("🔍  Search tools or T name..."))
