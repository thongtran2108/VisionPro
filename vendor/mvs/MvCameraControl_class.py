...
def _resolve_mvs_dll():
    """Tìm MvCameraControl.dll theo thứ tự ưu tiên (trả về đường dẫn file):
      1. ENV MVCAM_DLL_PATH        (đường dẫn file .dll HOẶC thư mục chứa nó)
      2. ENV MVCAM_COMMON_RUNENV   (chuẩn của MVS SDK)
      3. (đóng gói) <thư mục .exe>/dll/   — copy DLL cạnh VisionUltimate.exe
      4. (đóng gói) <_MEIPASS>/dll/        — PyInstaller gói vào _internal/dll/
      5. (source)   <repo>/dll/
      6. C:/Program Files (x86)/MVS/Development/Win64_x64/  (cài SDK mặc định)
    """
    dirs = []
    # 1. Override tường minh — chấp nhận cả file lẫn thư mục
    env_path = os.environ.get("MVCAM_DLL_PATH")
    if env_path:
        if os.path.isfile(env_path):
            return env_path
        dirs.append(env_path)
    # 2. Layout chuẩn của MVS SDK
    runenv = os.environ.get("MVCAM_COMMON_RUNENV")
    if runenv:
        arch = "Win64_x64" if sys.maxsize > 2**32 else "Win32_i86"
        dirs.append(os.path.join(runenv, arch))
    # 3 + 4. Khi chạy từ bản đóng gói PyInstaller
    if getattr(sys, "frozen", False):
        dirs.append(os.path.join(os.path.dirname(sys.executable), "dll"))
        meipass = getattr(sys, "_MEIPASS", None)
        if meipass:
            dirs.append(os.path.join(meipass, "dll"))
    # 5. Khi chạy từ source (dll/ cùng cấp main.py)
    here = os.path.dirname(os.path.abspath(__file__))
    dirs.append(os.path.normpath(os.path.join(here, "..", "..", "dll")))
    # 6. Cài MVS SDK mặc định
    dirs.append(r"C:\Program Files (x86)\MVS\Development\Win64_x64")
    ...
    raise FileNotFoundError(
        "MvCameraControl.dll không tìm thấy. Copy DLL (kèm các DLL phụ thuộc) "
        "vào folder 'dll/' cạnh VisionUltimate.exe, hoặc cài MVS SDK của "
        "HikRobot rồi đặt ENV MVCAM_COMMON_RUNENV.")


if sys.platform == "win32":
    _dll_path = _resolve_mvs_dll()
    # Python 3.8+ siết DLL search path: phải khai báo TƯỜNG MINH thư mục chứa
    # DLL để các DLL phụ thuộc của MVS SDK (FormatConversion.dll,
    # MediaProcess.dll, MVGigEVisionSDK.dll, MvUsb3vTL.dll...) load được.
    # Nếu không, dù tìm thấy MvCameraControl.dll vẫn lỗi "DLL load failed".
    _dll_dir = os.path.dirname(_dll_path)
    if hasattr(os, "add_dll_directory") and os.path.isdir(_dll_dir):
        try:
    ...
    raise OSError("MVS SDK chỉ chạy trên Windows.")
