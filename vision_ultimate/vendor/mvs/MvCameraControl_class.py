...
def _resolve_mvs_dll():
    """Tìm MvCameraControl.dll theo thứ tự ưu tiên:
      1. ENV var MVCAM_DLL_PATH (override hoàn toàn)
      2. ENV var MVCAM_COMMON_RUNENV (chuẩn của MVS SDK)
      3. <repo>/VisionPro/dll/MvCameraControl.dll
      4. C:/Program Files (x86)/MVS/Development/Win64_x64/MvCameraControl.dll (cài mặc định)
    """
    candidates = []
    if os.environ.get("MVCAM_DLL_PATH"):
        candidates.append(os.environ["MVCAM_DLL_PATH"])
    runenv = os.environ.get("MVCAM_COMMON_RUNENV")
    if runenv:
        arch = "Win64_x64" if sys.maxsize > 2**32 else "Win32_i86"
        candidates.append(os.path.join(runenv, arch, "MvCameraControl.dll"))
    here = os.path.dirname(os.path.abspath(__file__))
    candidates.append(os.path.normpath(os.path.join(here, "..", "..", "dll", "MvCameraControl.dll")))
    candidates.append(r"C:\Program Files (x86)\MVS\Development\Win64_x64\MvCameraControl.dll")
    for p in candidates:
        if p and os.path.isfile(p):
            return p
    raise FileNotFoundError(
        "MvCameraControl.dll không tìm thấy. Cài MVS SDK của HikRobot rồi đặt "
        "ENV MVCAM_COMMON_RUNENV, hoặc copy DLL vào VisionPro/dll/.")


if sys.platform == "win32":
    MvCamCtrldll = WinDLL(_resolve_mvs_dll())
else:
