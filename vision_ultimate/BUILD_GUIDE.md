...
REM Bat buoc:
pip install PySide6>=6.5.0 opencv-python>=4.8.0 numpy>=1.24.0 Pillow>=10.0.0 PyYAML
...
Output: `dist\VisionPro\VisionPro.exe`

Test: double-click `VisionPro.exe`. Nếu app khởi động và hiện startup picker → build thành công 🎉
...
3. Chạy `VisionPro.exe` từ terminal (`cmd`) — sẽ thấy error message rõ ràng.

Hoặc xem `dist\VisionPro\error.log` (runtime_hook đã setup auto-log stderr).
...
**Cách B — Bundle DLL kèm `.exe`** (chỉ làm nếu có quyền redistribute):
- Tạo folder `dll/` trong project (cùng cấp `main.py`).
- Copy `MvCameraControl.dll` (+ các DLL phụ thuộc như `FormatConversion.dll`, `MediaProcess.dll`...) vào `dll/`.
- Thêm vào `VisionPro.spec`:
  ```python
  datas = [
      ...
      ('dll/*.dll', 'dll'),
  ...
  ```
- Sửa `vendor/mvs/MvCameraControl_class.py` để ưu tiên `<exe_dir>/dll/`.
...
Tạm thời: user cần add exception trong Windows Defender cho folder `dist\VisionPro\`.
...
1. Chuột phải `VisionPro.exe` → Properties → Compatibility → Change high DPI settings.
...
Sau khi có `dist\VisionPro\`, dùng **Inno Setup** (free) để tạo file installer:
...
UninstallDisplayIcon={app}\VisionPro.exe
...
[Files]
Source: "dist\VisionPro\*"; DestDir: "{app}"; Flags: recursesubdirs

[Icons]
Name: "{group}\AOI Vision Pro"; Filename: "{app}\VisionPro.exe"
Name: "{commondesktop}\AOI Vision Pro"; Filename: "{app}\VisionPro.exe"

[Run]
Filename: "{app}\VisionPro.exe"; Description: "Run AOI Vision Pro now"; Flags: nowait postinstall skipifsilent
```
...
- [ ] Tạo file `README.txt` đặt cạnh `VisionPro.exe` ghi cách dùng cơ bản
